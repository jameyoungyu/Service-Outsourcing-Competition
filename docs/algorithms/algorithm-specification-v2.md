...
依据：`docs/innovation/differentiation-blueprint.md`（`INNO-1.0`）、`docs/experiments/identifiability-ablation.md`（`EXP-1.0`）
...
| 章节 | 对应 ALG-0.1 | 变更 | 理由 |
|---|---|---|---|
| §2 动态区间优选 | §6 | `[改]` 主口径改为 D-最优子模优选 | 加权权重不可辩护；能量大≠信息新（EXP-1.0 §3） |
| §3 持续激励 | 无 | `[增]` PE 阶次前置判据 | 回答"数据够不够辨识 n 阶模型" |
| §4 时滞估计 | §7 | `[改]` 候选生成改用预白化互相关 | 输出自相关制造伪峰 |
| §5 模型评价 | §11 | `[改]` 主指标改为自由仿真 FIT，增稳定性/增益/白度 | 一步预测 FIT 区分度仅为 1/27.7（EXP-1.0 §5） |
...
`tol` 默认 `1e-3`（`DEFAULT_EXCITATION_TOLERANCE`），在 S1–S6 上标定，复现教科书结论"m 次电平变化 → m 阶"（标定表见 `EXP-1.0` §6）。

**辨识 `(na, nb_j, nk_j)` 所需阶次**：`required_j = na + nb_j`。

**门限行为**：`PE_order(u_j) < required_j` 时，`fit_arx` 返回 `INSUFFICIENT_EXCITATION`，并在 details 中给出实际阶次、所需阶次和建议（补做激励试验 / 降低阶次 / 扩大数据范围）。这是一个**阻断性**结论，不是警告——用不满足激励条件的数据辨识出的模型没有意义。
