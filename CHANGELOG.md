...
### Added (2026-08-08，交付与知识产权材料)

- 产品使用手册 `docs/manual/用户手册.md`：一份材料同时满足两个用途——竞赛交付物「产品使用手册」，
  以及软著登记所需的「软件文档」。含安装、五分钟上手、逐页操作说明、
  「如何读懂系统给出的判断」（为什么主指标是自由仿真 FIT、条件数与秩状态怎么看、
  持续激励阶次为何只能当诊断）、错误码对照与**已知限制**七条；
- 软著登记材料 `docs/ip/software-copyright/`：登记申请信息（需人工确认项均标 `【待填】`）
  与 `scripts/build_copyright_listing.py` 生成的前 30 页 + 后 30 页源程序清单（每页 50 行，
  当前源程序 404 页 / 20,160 行）。脚本先扫描密钥、口令、连接串与私钥，检出即拒绝写出；
- 专利技术交底书 `docs/ip/patent/技术交底书.md`：**第 0 节明确写出 D-最优、CELF/惰性贪心、
  矩阵行列式引理均为既有技术，不得写成本项目的算法贡献**，并说明 `1-1/e` 保证只在数学前提
  满足时成立。可能可专利的是四个工程构造（门控前置及其分位数尺度、双停止条件、
  预算下限自动判定与提示、血缘元数据复用为缓存键）。第 4 节同时列出三条对本方案不利的实测。

### Added (2026-08-08，竞赛交付物)

- `docs/competition/项目概要.md`、`详细方案.md`、`分工与过程文档.md` 与 11 页答辩 PPT
  （`IndusOpt-A14-答辩.pptx`，生成脚本在 `slides/`，可重跑）；
- 三份材料一致地把「我们把标准答案实现了一遍然后发现它不成立」与「同时提交对自己不利的结果」
  放在显著位置，而不是只讲有利数据；
- 分工表与软著主体信息一律留 `【待填】`：涉及成员身份、权利归属与日期承诺，不由工具代填。

### Changed (2026-08-08，文档与实现对账)

- **`ALG-0.2` §3 的持续激励阻断口径被自己的实测推翻，据实修订。** 规范原文写"`PE_order < required`
  时 `fit_arx` 返回 `INSUFFICIENT_EXCITATION`，这是阻断性结论"。按此实现后立刻在真实形态数据上
  失败，测量表明该判据两个方向都会判错：标定容差 `1e-3` 下 8 电平输入报出阶次 1（远低于所需 4）
  而其模型自由仿真 FIT 有 95.82%；放松到 `1e-4` 后 3 电平输入达标而其模型 FIT 只有 0.21%，
  且教科书标定同时被破坏。**没有任何容差能把可用与不可用分开**，因此降级为诊断字段
  `validation.excitation_shortfall`，不阻断。真正阻断的仍是 `DESIGN_MATRIX_RANK_DEFICIENT`；
- `EXP-1.1` §6 的标定表补上适用范围：`1e-3` 复现的是**教科书信号上的教科书结论**，
  不代表该估计量在真实多电平输入上准确。

### Fixed (2026-08-08，文档与实现对账)

- **产品使用手册的错误码表里有 3 个码在代码中不存在**（`INSUFFICIENT_EXCITATION`、
  `FILE_TOO_LARGE`、`UNSUPPORTED_FILE_TYPE`）。用户按码查文档会查不到，或为不会发生的分支
  写处理逻辑。已改为实际会返回的码，并把表扩充到 14 条；
- **`ERR-1.0` 错误码字典与实现脱节**：阶段 4–11 新增的 10 个码从未写进字典，
  字典里另有 12 个是阶段 0/1 规划、代码中并不存在的。已升为 `ERR-1.1`，
  两类都如实标注，并说明 Agent 非法计划为何不走错误码（由合规校验拦截并出具证明，信息量更大）；
- 「13 个业务视图」在多处文档中已过时（新增自评测基准页后实为 14），阶段 1 的历史记录保留 13 不动。
...
- 阶段 4–11：后端 pytest `237 passed`（含 2 项端到端用例验收、21 项大模型集成测试、10 项
  加权分对照基线测试、7 项行预算/预算提示测试、4 项持续激励"上报而非阻断"测试，
  以及 2 项**真实 Redis + 真实 RQ Worker**往返验证），Ruff 与 Mypy 全部通过（69 个源文件）；
